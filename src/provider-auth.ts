...
const GLM_PROVIDERS = new Set(["zhipu", "zhipu-coding", "zhipu-coding-cn"]);
...
export async function verifyApiKey(
  ...
  if (!apiKey.includes(".")) {
    throw new Error(
      `${provider.label} API key is incomplete; copy the full id.secret value, including the dot`,
    );
  }
  // Coding Plan credentials are scoped to chat-completions. The dedicated
  // Coding endpoints can reject a valid plan key at /models, so validate them
  // when the user makes the first real request.

...
- slop: run one command line in the Slop build shell. It supports buffered pipes, stdout/stderr redirects, &&/|| lists, variables, command and arithmetic expansion, globbing, functions, and line-oriented if/for/while/case blocks. /bin includes make, sh, a compiled Git frontend, sed, basic ar, and bounded file utilities; cc/ld, Python, libgit2, browser curl, and GitHub transfer use browser host services. Each tool call uses a fresh shell — filesystem changes persist, shell variables and cwd do not (pass cwd; default /home/web). Use slop for shell workflows and Git/curl-like command lines.
